...
namespace CrankUp
{
    public partial class Levels : Node
    {
        ...
        /// <summary>
        /// This function sets up the level buttons and connects their signals to the appropriate functions.
        ...
        /// It enables or disables the buttons and sets the flag if gotted at least 1 star for the level.
        /// </summary>
        public void SetupLevelButtons()
        {
            ...
            {
                var button = GetNodeOrNull<TextureButton>($"Buttons/Level{i}");
                ...
                // The next playable level
                else if (i == 1 || (i > 1 && IsFlagVisible(i - 1)))
                {
                    button.Disabled = false;
                    numberLabel.Visible = true;
        ...
        /// <summary>
        /// This function checks if the flag icon should be visible for a specific level.
        /// </summary>
        /// <param name="level"></param>
        /// <returns></returns>
        public bool IsFlagVisible(int level)
        {
            var gameData = SaveSystem.GetGameData();
            if (gameData.LevelStars.TryGetValue(level, out int stars))
            {
                return stars > 0;
            }
            return false;
